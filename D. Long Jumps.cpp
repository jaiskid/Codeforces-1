...
int main()
{
    LL n,l,x,y,diff,i,a,b,c,d,e,sum,bal;
    ...
    while(cin>>n>>l>>x>>y)
    {
        ...
        {
            if(mp[x+y]) idx.pb(x);
            else if(cflag && bal+x<=l) idx.pb(bal+x);
            else if(bflag && sum-y>=0) idx.pb(sum-y);
            else if(bflag && sum+x<=l) idx.pb(sum+x);
            else if(dflag && diff-y>=0) idx.pb(diff-y);
            else if(dflag && diff+x<=l) idx.pb(diff+x);
            else idx.pb(x),idx.pb(y);
        }
        else if(!xflag)
        {
            idx.pb(x);
